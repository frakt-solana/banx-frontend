...
type FetchUserPortfolio = (props: {
  walletPubkey: string
  tokenType: LendingTokenType
  assetType: AssetType
}) => Promise<UserPortfolio | undefined>
export const fetchUserPortfolio: FetchUserPortfolio = async ({
  walletPubkey,
  tokenType,
  assetType,
}) => {
  const queryParams = new URLSearchParams({
    ...
    marketType: convertToMarketType(tokenType),
    tokenType: assetType,
  })

  const { data } = await axios.get<{ data: UserPortfolio }>(
    `${BACKEND_BASE_URL}/stats/portfolio/${walletPubkey}?${queryParams.toString()}`,
  )

  return await parseResponseSafe<UserPortfolio>(data.data, UserPortfolioSchema)
}
