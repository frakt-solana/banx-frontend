...
export const useSeasonUserRewards = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: createSeasonUserRewardsQueryKey(publicKeyString),
    queryFn: () => user.fetchSeasonUserRewards({ walletPubkey: publicKeyString }),
...
export const useLinkedWallets = () => {
  ...
  const { data: linkedWallets, isLoading } = useQuery({
    queryKey: createFetchLinkedWalletsQueryKey(publicKey?.toBase58() || ''),
    queryFn: () => user.fetchLinkedWallets({ walletPublicKey: publicKey?.toBase58() || '' }),
    refetchOnWindowFocus: false,
    enabled: !!publicKey,
    staleTime: 15 * 1000,
  })
