...
export const useLenderLoansState = create<SelectedTokenLoansState>((set, get) => ({
  selection: [],
  ...
  toggle: (loan, walletPublicKey) => {
    if (!walletPublicKey) return

    const { find, add, remove } = get()
    const isLoanInSelection = !!find(loan.publicKey, walletPublicKey)

    return isLoanInSelection ? remove(loan.publicKey, walletPublicKey) : add(loan, walletPublicKey)
  },
}))
